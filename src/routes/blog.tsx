...
import { ArrowRight, ChevronRight, Search, Calendar, Clock, Tag, Mail, Sparkles, BookOpen } from "lucide-react";
...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Industry insights, regulatory updates and stories from Thorneberry — Pakistan's global healthcare and pharmaceutical exporter." },
      { property: "og:title", content: "Insights & Journal — Thorneberry" },
      { property: "og:description", content: "Industry insights and stories from a global healthcare exporter." },
...
function BlogPage() {
  ...
  const filtered = useMemo(() => {
    return blogPosts.filter((p) => {
      ...
      const matchesQ = !ql || p.title.toLowerCase().includes(ql) || p.excerpt.toLowerCase().includes(ql) || p.tags.some((t) => t.toLowerCase().includes(ql));
      ...
      <section className="relative pt-40 pb-20 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          ...
          <div className="mt-5 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <h1 className="font-display text-5xl md:text-6xl text-white max-w-3xl leading-[1.05]">
              Insights from the <span className="text-transparent bg-clip-text brand-gradient">global healthcare</span> supply chain.
            </h1>
            <p className="text-white/65 max-w-md leading-relaxed">
              Field notes, regulatory updates and long-reads on pharmaceutical exports, surgical manufacturing and emerging medical technology.
            </p>
          </div>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-7xl px-6">
            <div className="flex items-end justify-between mb-10">
              ...
              <span className="text-xs text-white/50 uppercase tracking-[0.2em]">{latest.length} stories</span>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {latest.map((p, i) => <ArticleCard key={p.slug} post={p} index={i} />)}
            </div>
          </div>
        </section>
      )}
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 glass rounded-3xl p-8 md:p-10">
            <div className="flex items-center gap-2 text-[color:var(--teal)] text-xs uppercase tracking-[0.2em]"><BookOpen className="size-3.5" /> Most read</div>
            <h3 className="mt-3 font-display text-2xl md:text-3xl text-white">Popular this quarter</h3>
            {popular.length === 0 ? (
              <p className="mt-4 text-white/55 text-sm">Popular articles will appear here once our editorial team publishes the first stories.</p>
            ) : (
              <ul className="mt-8 divide-y divide-white/10">
                {popular.map((p, i) => (
                  <li key={p.slug}>
                    <Link to="/blog/$slug" params={{ slug: p.slug }} className="flex items-center gap-6 py-5 group">
                      <span className="font-display text-2xl text-white/30 group-hover:text-[color:var(--teal)] transition w-8">0{i + 1}</span>
                      <div className="flex-1">
                        <div className="text-white font-medium group-hover:text-[color:var(--teal)] transition">{p.title}</div>
                        <div className="text-xs text-white/50 mt-1">{p.category} · {p.readingMinutes} min read</div>
                      </div>
          ...
          <div className="relative overflow-hidden rounded-3xl p-8 md:p-10 glass-strong">
            ...
            <div className="relative">
              <div className="size-12 rounded-2xl brand-gradient grid place-items-center text-[color:var(--navy-deep)]"><Mail className="size-5" /></div>
              <h3 className="mt-5 font-display text-2xl text-white">The Thorneberry Briefing</h3>
              <p className="mt-2 text-white/65 text-sm leading-relaxed">A monthly newsletter on global pharma exports, regulatory shifts and supply-chain intelligence. No fluff.</p>
              ...
              <p className="mt-3 text-[11px] text-white/40">We respect your inbox. Unsubscribe anytime.</p>
            </div>
          </div>
        </div>
      </section>
            ...
            <div className="grid lg:grid-cols-2 gap-0">
              ...
              <div className="p-8 md:p-12 flex flex-col justify-center">
                <div className="flex items-center gap-2 text-xs">
                  <span className="px-3 py-1 rounded-full brand-gradient text-[color:var(--navy-deep)] font-semibold uppercase tracking-widest">Featured</span>
                  <span className="text-white/55">{post.category}</span>
                </div>
                <h2 className="mt-5 font-display text-3xl md:text-4xl text-white leading-tight group-hover:text-[color:var(--teal)] transition">{post.title}</h2>
                ...
                <div className="mt-6 flex items-center gap-5 text-xs text-white/55">
                  <span className="flex items-center gap-1.5"><Calendar className="size-3.5" /> {formatDate(post.publishedAt)}</span>
                  <span className="flex items-center gap-1.5"><Clock className="size-3.5" /> {post.readingMinutes} min read</span>
                </div>
                <span className="mt-8 inline-flex items-center gap-2 text-[color:var(--teal)] font-medium">Read article <ArrowRight className="size-4 group-hover:translate-x-1 transition" /></span>
              </div>
            </div>
          </div>
        </Link>
...
function ArticleCard({ post, index }: { post: BlogPost; index: number }) {
  return (
    <motion.article initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (index % 3) * 0.06 }}>
      <Link to="/blog/$slug" params={{ slug: post.slug }} className="group glass rounded-3xl overflow-hidden block hover:-translate-y-1 transition">
        <div className="relative h-52 overflow-hidden">
          <img src={post.cover} alt={post.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-110 transition duration-700" />
          <div className="absolute inset-0 bg-gradient-to-t from-[color:var(--navy-deep)] via-transparent to-transparent" />
          <span className="absolute top-4 left-4 text-[11px] uppercase tracking-widest px-2.5 py-1 rounded-full glass-strong text-white">{post.category}</span>
        </div>
        <div className="p-6">
          ...
          <h3 className="mt-3 font-display text-xl text-white leading-snug group-hover:text-[color:var(--teal)] transition">{post.title}</h3>
          <p className="mt-2 text-sm text-white/60 leading-relaxed line-clamp-2">{post.excerpt}</p>
          <span className="mt-5 inline-flex items-center gap-1.5 text-sm text-[color:var(--teal)]">Read more <ArrowRight className="size-4 group-hover:translate-x-1 transition" /></span>
        </div>
      </Link>
    </motion.article>
  );
      ...
      <div className="mx-auto max-w-5xl px-6">
        <div className="relative overflow-hidden rounded-[2rem] glass-strong p-12 md:p-16 text-center">
          ...
          <div className="relative">
            ...
            <h2 className="mt-6 font-display text-3xl md:text-4xl text-white">The journal is launching soon.</h2>
            <p className="mt-4 text-white/65 max-w-xl mx-auto leading-relaxed">
              We're preparing in-depth articles on pharmaceutical exports, surgical manufacturing in Sialkot, regulatory pathways and global trade. Subscribe to be notified when the first stories go live.
            </p>
            <form onSubmit={(e) => e.preventDefault()} className="mt-8 max-w-md mx-auto glass rounded-full p-1.5 flex items-center">
              <input type="email" required placeholder="you@company.com" className="flex-1 bg-transparent px-4 py-2 text-sm text-white placeholder:text-white/40 outline-none" />
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
              {blogCategories.map((c) => (
                <span key={c} className="text-[11px] uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/10 text-white/60 flex items-center gap-1.5">
