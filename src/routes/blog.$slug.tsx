    ...
    <SiteLayout>
      <section className="pt-40 pb-24">
        <div className="mx-auto max-w-3xl px-6 text-center">
          <div className="inline-flex items-center gap-2 glass px-3 py-1 rounded-full text-xs text-[color:var(--teal)] uppercase tracking-[0.2em]">404 — Article</div>
          <h1 className="mt-5 font-display text-4xl md:text-5xl text-white">Article not found</h1>
          <p className="mt-4 text-white/65">This story isn't published yet. Browse the journal for available articles.</p>
          <Link to="/blog" className="mt-8 inline-flex items-center gap-2 brand-gradient text-[color:var(--navy-deep)] font-semibold px-6 py-3 rounded-full">
  ...
  errorComponent: ({ reset }) => (
    <SiteLayout>
      <section className="pt-40 pb-24 text-center text-white">
        ...
        <button onClick={() => reset()} className="mt-6 brand-gradient text-[color:var(--navy-deep)] font-semibold px-6 py-3 rounded-full">Retry</button>
      </section>
    </SiteLayout>
  ),
  ...
  return (
    <SiteLayout>
      ...
      <div className="fixed top-0 left-0 right-0 h-1 z-[60] bg-white/5">
        <div className="h-full brand-gradient transition-[width] duration-100" style={{ width: `${progress}%` }} />
      </div>
      ...
      <section className="relative pt-36 pb-16 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-6">
          ...
          <span className="text-[11px] uppercase tracking-widest px-3 py-1 rounded-full brand-gradient text-[color:var(--navy-deep)] font-semibold">{post.category}</span>
          <h1 className="mt-5 font-display text-4xl md:text-6xl text-white leading-[1.05]">{post.title}</h1>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-6 text-sm text-white/65">
            <span className="flex items-center gap-2"><User className="size-4 text-[color:var(--teal)]" /> {post.author.name}</span>
            <span className="flex items-center gap-2"><Calendar className="size-4 text-[color:var(--teal)]" /> {formatDate(post.publishedAt)}</span>
            <span className="flex items-center gap-2"><Clock className="size-4 text-[color:var(--teal)]" /> {post.readingMinutes} min read</span>
          </div>
        </div>
      </section>
      ...
      <section className="pb-24">
        <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-[1fr_280px] gap-12">
          <article className="prose-article text-white/80 leading-relaxed text-[17px] space-y-6"
            dangerouslySetInnerHTML={{ __html: post.content }} />

          <aside className="space-y-6 lg:sticky lg:top-28 self-start">
            <div className="glass rounded-2xl p-6">
              <div className="text-xs uppercase tracking-[0.2em] text-[color:var(--teal)]">Share</div>
              <div className="mt-4 flex gap-2">
                {[Twitter, Linkedin, Facebook, Link2].map((Icon, i) => (
                  <button key={i} className="grid place-items-center size-10 rounded-full glass-strong text-white/80 hover:text-white hover:bg-white/10 transition" aria-label="Share">
            ...
            <div className="glass-strong rounded-2xl p-6">
              <div className="size-10 rounded-xl brand-gradient grid place-items-center text-[color:var(--navy-deep)]"><Mail className="size-4" /></div>
              <h4 className="mt-4 text-white font-medium">Subscribe to the Briefing</h4>
              <p className="mt-1.5 text-xs text-white/60">Monthly journal updates, straight to your inbox.</p>
              <form onSubmit={(e) => e.preventDefault()} className="mt-4 flex flex-col gap-2">
                <input type="email" required placeholder="you@company.com" className="bg-white/5 border border-white/10 rounded-full px-4 py-2.5 text-xs text-white placeholder:text-white/40 outline-none focus:border-[color:var(--teal)]" />
                <button className="brand-gradient text-[color:var(--navy-deep)] font-semibold text-xs px-4 py-2.5 rounded-full">Subscribe</button>
              </form>
            </div>
          </aside>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-6">
          <div className="glass-strong rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              <div className="text-xs uppercase tracking-[0.2em] text-[color:var(--teal)]">Written by</div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {related.map((p) => (
                <Link key={p.slug} to="/blog/$slug" params={{ slug: p.slug }} className="group glass rounded-3xl overflow-hidden hover:-translate-y-1 transition">
                  ...
                  <div className="p-6">
                    <div className="text-[11px] uppercase tracking-widest text-[color:var(--teal)]">{p.category}</div>
                    <h3 className="mt-2 font-display text-lg text-white group-hover:text-[color:var(--teal)] transition">{p.title}</h3>
                    <span className="mt-4 inline-flex items-center gap-1.5 text-sm text-white/70">Read <ArrowRight className="size-4 group-hover:translate-x-1 transition" /></span>
                  </div>
                </Link>
              ))}
            </div>
